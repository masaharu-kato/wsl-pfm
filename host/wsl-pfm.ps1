...
Write-Host "-------------------------------------------- "
Write-Host "    wsl-pfm: WSL Port Forwarding Manager "
Write-Host "-------------------------------------------- "
...
function refreshDistroPortForwards($distro) {
    ...
    foreach($pf in $pf_reqs) {
        ...
        if(($old_ip -ne $new_ip) -or (!$old_forwarded_ports[$conPort])) {
            ...
            Write-Host "[$distro]   Setting port forwarding *:$lisPort -> ${new_ip}:$conPort ..."
            ...
            try {
                ...
                Write-Host "[$distro]    -> Successfully done."

            } catch {
                Write-Host "[$distro]    -> Failed: $_"
            }
    ...
    foreach($conPort in $old_forwarded_ports.Keys) {
        ...
        if($lisPort) {
            Write-Host "[$distro]   Delete port forwarding  *:$lisPort -> ${old_ip}:$conPort"
...
# Write-Host "REGEX_DISTRO_PATH: $REGEX_DISTRO_PATH"
...
$action = {
    ...
    # Write-Host "$(Get-Date)`t$changeType`t$path"
    if(($path -match "(\\ip|\\ports\\\d+\\\d+)")) {
        try {
            onFileChanged $path
        } catch {
            Write-Host 'An error has occured inside onFileChanged().'
            Write-Host $_
        }
    }
}
...
Write-Host "Start file watch jobs ..."
...
try {
        ...
        foreach($distro in $distroList) {
            if($distroRefreshRequested[$distro]) {
                ...
                if($distroRefreshNeeded[$distro]) {
                    Write-Host "[$distro] Refresh port forwards ..."
                    try {
                        refreshDistroPortForwards $distro
                        Write-Host "[$distro]  -> Done."
                    } catch {
                        Write-Host "[$distro]  -> Failed: $_"
                    }
                }
    ...
    Write-Host "Removing file watch jobs ..."
